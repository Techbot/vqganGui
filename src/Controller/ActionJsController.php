...
#[Route('/actionjs')]
class ActionJsController extends AbstractController
{
    #[Route('/', name: 'app_actionJs_index', methods: ['GET'])]
    public function index(ActionRepository $actionRepository): Response
    {
        $payload =  $actionRepository->findAll();
        rsort($payload);
    ...
    public function new(Request $request, ActionRepository $actionRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $actionRepository->add($action, true);
